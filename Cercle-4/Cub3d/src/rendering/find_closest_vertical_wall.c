...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/25 17:01:42 by aoger             #+#    #+#             */
/*   Updated: 2025/04/25 17:01:44 by aoger            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (ft_is_facing_right(ray->angle))
		*dist_x_next_inter = BLOCK_SIZE;
	else
		*dist_x_next_inter = -BLOCK_SIZE;
	*dist_y_next_inter = *dist_x_next_inter * tan(ray->angle);
}
...
double *first_hit_x, double *first_hit_y)
{
	...
	if (ft_is_facing_right(ray->angle))
		*first_hit_x = floor(player->x / BLOCK_SIZE) * BLOCK_SIZE \
			+ BLOCK_SIZE;
	else
		*first_hit_x = floor(player->x / BLOCK_SIZE) * BLOCK_SIZE - 0.001;
	dist_x = *first_hit_x - player->x;
	*first_hit_y = player->y + dist_x * tan(ray->angle);
}
